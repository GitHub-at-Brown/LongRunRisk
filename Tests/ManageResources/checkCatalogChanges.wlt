BeginTestSection["checkCatalogChanges Tests"]
Begin["FernandoDuarte`LongRunRisk`Tests`ManageResources`checkCatalogChanges`"]

(* --- merged from: checkCatalogChanges_test1.wlt --- *)
...
VerificationTest[
  ...
  TestID -> "checkCatalogChanges-assoc-input-returns-structure@@Tests/ManageResources/checkCatalogChanges.wlt:16,1-65,2"
]

(* --- merged from: checkCatalogChanges_test2.wlt --- *)
(* Setup: Load ManageResources package *)
Off[General::shdw];
Needs["FernandoDuarte`LongRunRisk`Tools`ManageResources`"];
On[General::shdw];
...
VerificationTest[
  ...
  True,
  TimeConstraint -> $timeLimit,
  TestID -> "checkCatalogChanges-detects-changed-new-removed@@Tests/ManageResources/checkCatalogChanges.wlt:79,1-135,2"
]
