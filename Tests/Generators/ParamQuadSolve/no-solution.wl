(* Auto-generated from: /Users/fduarte/Library/CloudStorage/Dropbox-Personal/MyPackages/LongRunRisk-feat-gen-tests/Tests/ParamQuadSolve/no-solution.wlt
   by WLTTestRefactor` on 2026-01-03
   DO NOT EDIT BY HAND. Edit the generator or the original sources instead.
*)

BeginPackage["WLTGenerated`nosolution`"];
...
$sourceExpressions = {
    HoldComplete[BeginTestSection["no-solution Tests"]],
    HoldComplete[Begin["FernandoDuarte`LongRunRisk`Tests`ParamQuadSolve`noSolution`"]],
    ...
    HoldComplete[VerificationTest[Module[{x, y, eq, vars, r}, eq = {x == 0, x == 1, y == 0}; vars = {x, y}; r = pqs[eq, vars, "ValidationOption" -> True]; AssociationQ[r]], True, {}, TestID -> "assoc@@Tests/ParamQuadSolve/no-solution.wlt:10,1-20,2"]],
    HoldComplete[VerificationTest[Module[{x, y, eq, vars, r, leftover, ver}, eq = {x == 0, x == 1, y == 0}; vars = {x, y}; r = pqs[eq, vars, "ValidationOption" -> True]; leftover = r["Diagnostics"]["LeftoverEquations"]; ver = r["Verification"]; Length[leftover] >= 1 || AnyTrue[Flatten[ver], FalseQ]], True, {}, TestID -> "signaled-no-solution@@Tests/ParamQuadSolve/no-solution.wlt:22,1-34,2"]],
    HoldComplete[End[]],
...
Options[GenerateWLT] = {
    ...
    "TestIDPrefix" -> "no-solution",
    "HashLength" -> 8,
    "TargetRelativeWLTPath" -> "../../ParamQuadSolve/no-solution.wlt"
};
