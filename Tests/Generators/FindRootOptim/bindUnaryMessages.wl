(* Auto-generated from: /Users/fduarte/Library/CloudStorage/Dropbox-Personal/MyPackages/LongRunRisk-feat-gen-tests/Tests/FindRootOptim/bindUnaryMessages.wlt
   by WLTTestRefactor` on 2026-01-03
   DO NOT EDIT BY HAND. Edit the generator or the original sources instead.
*)

BeginPackage["WLTGenerated`bindUnaryMessages`"];
...
$sourceExpressions = {
    HoldComplete[BeginTestSection["bindUnaryMessages Tests"]],
    HoldComplete[Begin["FernandoDuarte`LongRunRisk`Tests`FindRootOptim`bindUnaryMessages`"]],
    HoldComplete[Needs["FernandoDuarte`LongRunRisk`Tools`FindRootOptim`"]; ],
    HoldComplete[bindUnary = ToExpression["FernandoDuarte`LongRunRisk`Tools`FindRootOptim`bindUnary"]; ],
    HoldComplete[buildKernel = ToExpression["FernandoDuarte`LongRunRisk`Tools`FindRootOptim`buildKernel"]; ],
    HoldComplete[timeLimit = 5; ],
    HoldComplete[ToExpression["FernandoDuarte`LongRunRisk`Tests`FindRootOptim`signA"]; ],
    HoldComplete[Module[{expr, vars, params, signA}, signA = ToExpression["FernandoDuarte`LongRunRisk`Tests`FindRootOptim`signA"]; gamma = ToExpression["FernandoDuarte`LongRunRisk`Tests`FindRootOptim`gamma"]; delta = ToExpression["FernandoDuarte`LongRunRisk`Tests`FindRootOptim`delta"]; expr = signA[1]*gamma + signA[2]*delta - A[0]; vars = {A[0]}; params = {gamma, delta}; kernel = buildKernel[expr, vars, params, "CompileSignSymbol" -> "signA"]; ]; ],
    HoldComplete[gamma = ToExpression["FernandoDuarte`LongRunRisk`Tests`FindRootOptim`gamma"]; ],
    HoldComplete[delta = ToExpression["FernandoDuarte`LongRunRisk`Tests`FindRootOptim`delta"]; ],
    HoldComplete[VerificationTest[bindUnary[kernel, Association[gamma -> 1., delta -> 2.], {1}] === $Failed, True, {FernandoDuarte`LongRunRisk`Tools`FindRootOptim`bindUnary::toofewsigns}, TimeConstraint -> timeLimit, TestID -> "toofewsigns-returns-failed-one-sign@@Tests/FindRootOptim/bindUnaryMessages.wlt:30,1-36,4"]],
    HoldComplete[Needs["FernandoDuarte`LongRunRisk`Tools`FindRootOptim`"]; ],
    HoldComplete[bindUnary = ToExpression["FernandoDuarte`LongRunRisk`Tools`FindRootOptim`bindUnary"]; ],
    HoldComplete[buildKernel = ToExpression["FernandoDuarte`LongRunRisk`Tools`FindRootOptim`buildKernel"]; ],
    HoldComplete[timeLimit = 5; ],
    ...
    HoldComplete[gamma = ToExpression["FernandoDuarte`LongRunRisk`Tests`FindRootOptim`gamma"]; ],
    HoldComplete[delta = ToExpression["FernandoDuarte`LongRunRisk`Tests`FindRootOptim`delta"]; ],
    HoldComplete[VerificationTest[bindUnary[kernel, Association[gamma -> 1., delta -> 2.], {}] === $Failed, True, {FernandoDuarte`LongRunRisk`Tools`FindRootOptim`bindUnary::toofewsigns}, TimeConstraint -> timeLimit, TestID -> "toofewsigns-returns-failed-empty-signs@@Tests/FindRootOptim/bindUnaryMessages.wlt:64,1-70,4"]],
    HoldComplete[Needs["FernandoDuarte`LongRunRisk`Tools`FindRootOptim`"]; ],
    HoldComplete[bindUnary = ToExpression["FernandoDuarte`LongRunRisk`Tools`FindRootOptim`bindUnary"]; ],
    HoldComplete[buildKernel = ToExpression["FernandoDuarte`LongRunRisk`Tools`FindRootOptim`buildKernel"]; ],
    HoldComplete[timeLimit = 5; ],
    ...
    HoldComplete[gamma = ToExpression["FernandoDuarte`LongRunRisk`Tests`FindRootOptim`gamma"]; ],
    HoldComplete[delta = ToExpression["FernandoDuarte`LongRunRisk`Tests`FindRootOptim`delta"]; ],
    HoldComplete[VerificationTest[Module[{result}, result = bindUnary[kernel, Association[gamma -> 1., delta -> 2.], {1, -1}]; MatchQ[result, {_Function, _Function}]], True, {}, TimeConstraint -> timeLimit, TestID -> "valid-signs-succeeds@@Tests/FindRootOptim/bindUnaryMessages.wlt:98,1-107,4"]],
    HoldComplete[End[]],
...
Options[GenerateWLT] = {
    ...
    "TestIDPrefix" -> "bindUnaryMessages",
    "HashLength" -> 8,
    "TargetRelativeWLTPath" -> "../../FindRootOptim/bindUnaryMessages.wlt"
};
