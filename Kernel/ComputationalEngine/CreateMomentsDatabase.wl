...
createDatabase[
	...
	Module[
		...
		With[
		{
			...
			covLong=Symbol["FernandoDuarte`LongRunRisk`ComputationalEngine`CreateMomentsDatabase`covLong"<>modelShortName]
		},
...
FullForm]\)&/@{q}) ;(*at least one input argument is non-zero*)
		...
		covLong[v1_,{v2_ ,v3_},q1_Integer,q2_Integer,i__]/;((countStockVars[{v1, v2, v3},"dd"|"pd"|"ret"]===Length[List[i]])&&(Length[List[i]]>0)):=covLong[v1,{v2,v3},q1,q2,i]=Module[{pos,vdd,tc},
		...
		covLong[{v1_ ,v2_},v3_,q1_Integer,q2_Integer,i__]/;((countStockVars[{v1, v2, v3},"dd"|"pd"|"ret"]===Length[List[i]])&&(Length[List[i]]>0)):=covLong[{v1,v2},v3,q1,q2,i]/;((countStockVars[{v1, v2, v3},"dd"|"pd"|"ret"]===Length[List[i]])&&(Length[List[i]]>0))=Module[{pos,vdd,tc},
		pos=positionStockVars[{{v1 ,v2},v3},"dd"|"pd"|"ret"];
		...
		covLong[{v1_,v2_},{v3_ ,v4_},q1_Integer,q2_Integer,q3_Integer,i__]/;((countStockVars[{v1, v2, v3, v4},"dd"|"pd"|"ret"]===Length[List[i]])&&(Length[List[i]]>0)):=covLong[{v1,v2},{v3,v4},q1,q2,q3,i]=Module[{pos,vdd,tc},
...
ShowStringCharacters->True,
NumberMarks->True],
FullForm]\))/;(!FreeQ[HoldPattern[b],Count] && FreeQ[HoldPattern[c],Module]):>(RuleDelayed[HoldPattern[a],c]),{0,Infinity}]&/@DownValues[covLong];
		(*shocks*)
		Do[
			v=shocksList[[mm]];
				covLong[v,0]=1;
		...
		DownValues[covLong]=DeleteDuplicates[Flatten[ParallelEvaluate[DownValues[covLong]]]];
		(* dd *)
		Do[
			(* one stock *)
			v1=varListdd[[jj]];
		...
		Do[
			Do[
				...
				DownValues[covLong]=DeleteDuplicates[Flatten[ParallelEvaluate[DownValues[covLong]]]];
				(*remove function evaluation condition in memoized DownValues of covLong*)
				DownValues[covLong]=Replace[#,(RuleDelayed[HoldPattern[Verbatim[HoldPattern][Verbatim[Condition][a__,b__]]],c__])/;(!FreeQ[HoldPattern[b],Count] && FreeQ[HoldPattern[c],Module])->HoldPattern[a]:>c,{0,Infinity}]&/@DownValues[covLong];
				,
